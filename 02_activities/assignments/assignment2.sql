...
ranked_sales AS (
	...
	RANK() OVER (ORDER BY total_sales ASC) AS asc_sales_rank,
    RANK() OVER (ORDER BY total_sales DESC)  AS desc_sales_rank
	FROM daily_sales
)

SELECT market_date, total_sales, 'lowest' AS sales_rank
FROM ranked_sales
WHERE asc_sales_rank = 1 
	
UNION

SELECT market_date, total_sales, 'highest' AS sales_rank
FROM ranked_sales
WHERE desc_sales_rank = 1;
...
SELECT vendor_name, product_name, sum(price) AS total
FROM (
	SELECT DISTINCT
	vendor_name, product_name, 5 * original_price as price
...
SET current_quantity = (
  ...
  FROM (
    ...
    LEFT JOIN (
      SELECT *,
             ROW_NUMBER() OVER (PARTITION BY vi.product_id ORDER BY market_date DESC) AS right_now
      FROM vendor_inventory vi 
    ) vi ON p.product_id = vi.product_id
    WHERE right_now = 1 OR right_now IS NULL
  ) p
