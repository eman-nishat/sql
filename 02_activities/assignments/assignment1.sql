...
SELECT product_id, product_name,
	...
	ELSE 'bulk'
	END AS prod_qty_type_condensed,
	CASE WHEN product_name LIKE '%peppers%'
	THEN '1'
	ELSE '0'
	END AS pepper_flag
FROM product;
...
SELECT customer.customer_id, customer.customer_first_name, customer.customer_last_name, 
SUM(customer_purchases.quantity*customer_purchases.cost_to_customer_per_qty) AS total_purchases
FROM customer
...
strftime ('%m', market_date) AS purchase_month,
strftime ('%Y', market_date) AS purchase_year,
quantity*cost_to_customer_per_qty AS money_spent
FROM customer_purchases
